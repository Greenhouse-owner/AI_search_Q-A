...
def index_and_search_documents():
    ...
    try:
        client = Elasticsearch(
            "http://localhost:9200",  # 1. 使用 https
            basic_auth=("elastic", os.getenv('ELASTICSEARCH_PASSWORD', 'your_elastic_password')),  # 替换为实际密码')),  # 从环境变量获取密码
        ...
        if os.path.isfile(file_path):
            try:
                ...
                if content.strip():
                    ...
                    client.index(
                        index=index_name,
                        id=str(doc_id_counter),  # 保持为字符串类型
                        document={
                            "file_name": filename,
                            "content": content
                        }
