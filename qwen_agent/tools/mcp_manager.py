...
class MCPManager:
    ...
    def __init__(self):
        global _Tool, _ClientSession, _StdioServerParameters, _stdio_client, _sse_client, _streamablehttp_client
        ...
        for server_name in mcp_servers:
            ...
            if client.resources:
                ...
                try:
                    # 检查session是否可用
                    if client.session:
                        # 先检查服务器是否支持list_resource_templates方法
                        if hasattr(client.session, 'list_resource_templates'):
                            ...
                            logger.info(f"Server {server_name} does not support list_resource_templates method")
...
class MCPClient:
        ...
        try:
            if 'url' in mcp_server:
                ...
                if mcp_server.get('type', 'sse')=='streamable-http':
                    ...
                    """
                    self._streams_context = _streamablehttp_client(url=url)
                    read_stream, write_stream, get_session_id = await self.exit_stack.enter_async_context(self._streams_context)
                    self._session_context = _ClientSession(read_stream, write_stream)
                    ...
                    self._streams_context = _sse_client(url, headers)
                    streams = await self.exit_stack.enter_async_context(self._streams_context)
                    self._session_context = _ClientSession(*streams)
                    self.session = await self.exit_stack.enter_async_context(self._session_context)
                ...
                stdio_transport = await self.exit_stack.enter_async_context(_stdio_client(server_params))
                self.stdio, self.write = stdio_transport
                self.session = await self.exit_stack.enter_async_context(_ClientSession(self.stdio, self.write))
            ...
            try:    
                if self.session:
                    # 先检查服务器是否支持list_resources方法
                    if hasattr(self.session, 'list_resources'):
                        list_resources = await self.session.list_resources()
                        # 检查资源列表是否不为空
                        if hasattr(list_resources, 'resources') and list_resources.resources:
                            self.resources = True
                    else:
                        logger.info(f"Server {mcp_server_name} does not support list_resources method")
            except Exception as e:
    ...
    async def execute_function(self, tool_name, tool_args: dict):
        ...
        if tool_name == 'list_resources':
            try:
                # 检查服务器是否支持list_resources方法
                if not hasattr(self.session, 'list_resources'):
                    return "Error: Server does not support list_resources method"
            ...
            try:
                # 检查服务器是否支持read_resource方法
                if not hasattr(self.session, 'read_resource'):
                    return "Error: Server does not support read_resource method"
            ...
            # 检查服务器是否支持call_tool方法
            if not hasattr(self.session, 'call_tool'):
                return f"Error: Server does not support calling tool '{tool_name}'"
